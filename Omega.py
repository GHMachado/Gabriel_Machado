# -*- coding: utf-8 -*-
"""
Created on Wed Dec 13 22:48:53 2023
...
ds = xr.open_dataset("C:/Users/gabri/datasets/verticalV.nc")
...
for index, row in df.iterrows():
    ...
    w_specific = w.sel(latitude=latitude, longitude=longitude, time=time_index, method='nearest')
    ...
    axs.semilogy(w_specific, w_specific['level'], linestyle='--', marker='o', label='line with marker', color='b')
    axs.grid(True)
    axs.axvline(0, color='r')
    
    # Títulos e legendas
    axs.set_title(f'Ômega em ({latitude}, {longitude}) - {time_index}', fontsize='16')
    axs.set_ylabel('Níveis de pressão (hPa)', fontsize='16')
    axs.set_xlabel('Ômega (Pa/s)', fontsize='16')
